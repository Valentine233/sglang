...
class XPUAttentionBackend(AttentionBackend):
    ...
    def _cascade_expand_attn(
        ...
        Args:
            q:             ``(num_rows, Hq, D)`` -- exactly one query per row.
            k_flat/v_flat: ``(total_slots, Hk, D/Dv)`` flat, token-slot indexed.
            page_table:    ``(num_rows, M)`` token-slot indices, valid entries
                           packed first per row.
            cache_seqlens: ``(num_rows,)`` per-row count of valid branch slots.

        Returns:
            o_expand:   ``(num_rows, Hq, Dv)`` in ``q`` dtype.
            lse_expand: ``(num_rows, Hq)`` float32, natural-log domain (matches the
                        transposed ``softmax_lse`` the paged kernel would return).
        """
        num_rows = q.shape[0]
        ...
        cu_seqlens_q = torch.arange(
            0, num_rows + 1, dtype=torch.int32, device=q.device
        )
